...
class FindIncludes:
    ...
    def find_includes(self):
        ...
        for inc in missing_incs:
            if inc.name in self.header_files:
...
parser.add_argument("-i", "--ignore_sys", action="store_true", help="Ignore system header files")
parser.add_argument("-m", "--missing_headers", action="store_true", help="Print missing header files only")
parser.add_argument('-p', '--print_locations', action='store_true', help="Print file locations and line numbers for references to include statements")
parser.add_argument('-pp', '--print_separate', action='store_true', help="Print file locations after listing discovered header files")
